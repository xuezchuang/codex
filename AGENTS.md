...
The xcode binary is strictly isolated from upstream codex at the config layer:

- The xcode binary reads **only** the `XCODE_HOME` environment variable. It does
  **not** read `CODEX_HOME`. If you have `CODEX_HOME` set for the upstream
  binary, it will not leak into xcode.
- If `XCODE_HOME` is unset, xcode defaults to `~/.xcode` (note: `.xcode`, not
  `.codex`). The upstream binary defaults to `~/.codex`. The two binaries
  therefore never share a config directory by accident.
- `xcode.ps1` (at the repo root) sets `XCODE_HOME=~/.xcode` and runs
  `cargo run --bin codex` from `codex-rs/`. Use this script to launch xcode;
  it does not set `CODEX_HOME`, so running it does not interfere with any
  upstream `CODEX_HOME` you may have set elsewhere.
- `find_codex_home` in `codex-rs/utils/home-dir/src/lib.rs` is the single
  source of truth for this resolution. Do not read `CODEX_HOME` from anywhere
  else in the xcode fork.

If upstream ever adds its own changes to `find_codex_home`, rebase them in
(`git rebase master` from `xcode`). Expect minor conflict on the env-var
branch; the resolution is always "keep xcode's XCODE_HOME-only behavior".
